...
def plot_assumption_free(scores, data, bins=50):
    ...
    vlin = linspace(mu-3*sigma, mu+3*sigma, 13)
    colors = linspace(0, 1, 256)[::int(256/((len(vlin)-1)/2))][:(len(vlin)-1)/2]
    colors = [(c,0,0) for c in colors]+[(1,1,1)]+[(0,c,0) for c in reversed(colors)]
