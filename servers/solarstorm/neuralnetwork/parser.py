        ...
        for year in range(2001, 2013):
            with open("{0}/dsd_{1}.txt".format(self.src, year), 'r') as in_txt:
                ...
                for line in in_txt:
                    ...
                    if self.index == 0:
                        # For the first instance, using only the inputs.
                        # All values preceded by 'p' are meant to refer
                        # the previous day values i.e. inputs
                        ...
                        # Write the csv data with shifted output
                        out_csv.writerow(row_data)

                        # Updating the 'p' values to current values
                        self.pdate = '-'.join(values[:3])
                        self.pradioflux = values[3]
                        self.psunspotnum = values[4]
                        self.psunspotarea = values[5]
