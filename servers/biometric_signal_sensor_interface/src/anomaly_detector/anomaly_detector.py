...
class AnomalyDetector(object):
    ...
    def af_anomaly_detect(self, rr_intervals, hr_quality_indices):
        ...
        if not (len(rr_intervals)) == self.window_size:
            raise ValueError("window length of rr_intervals passed doesn't match config file")

        if not (rr_intervals['hexoskin_timestamps'][0] >= hr_quality_indices['hexoskin_timestamps'][0] and 
            rr_intervals['hexoskin_timestamps'][len(rr_intervals)-1] <= hr_quality_indices['hexoskin_timestamps'][len(hr_quality_indices)-1]):
            raise ValueError("first rr_interval timestamp and last rr_interval timestamp must lie within first and last timestamp of hr_quality")

        AF = AtrialFibrillation(rr_intervals, hr_quality_indices, self.window_size)
        return AF.get_anomaly()


def main():
    AD = AnomalyDetector()
    rr_intervals = (pd.read_csv('rrinterval.txt', sep="\t", nrows=AD.window_size,
                    dtype={"hexoskin_timestamps":np.int64, "rr_int":np.float64}, header=None, 
                    names=["hexoskin_timestamps", "rr_int"]))
    hr_quality_indices = (pd.read_csv('hr_quality.txt', sep="\t", nrows=AD.window_size-8, 
                        dtype={"hexoskin_timestamps":np.int64, "quality_ind":np.int32}, 
                        header=None, names=["hexoskin_timestamps", "quality_ind"]))
