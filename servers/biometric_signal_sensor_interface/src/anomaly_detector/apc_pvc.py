...
class APC(object):
    ...
    def checkAorV(self, temp_beatlist):
        ...
        # get the QRSareas
        QRSarea_list =\
            self.get_window(len(temp_beatlist), 0,
                            temp_beatlist[len(temp_beatlist)-1][0] + 1, 2)
        # get the vecg values
        vecg_list = self.get_window(len(temp_beatlist), 0,
                                    temp_beatlist[len(temp_beatlist)-1][0] + 1,
                                    3)
    ...
    def supraventricular_tachycardia(self, timestamp):
        ...
        if rrint[0][1] < 0.5 * 256:
            if not (self.QRSarea_REF and self.vecg_REF):
                ...
                prev_five_QRSarea =\
                    sorted(prev_five_QRSarea, key=lambda x: x[1])
                ...
                QRSarea_DIFF =\
                    (abs(QRSarea - self.QRSarea_REF)/self.QRSarea_REF)*100

                vecg = self.get_window(1, 0, timestamp, 3)[0][1]
                vecg_DIFF = abs(vecg - self.vecg_REF)

                # if QRSArea difference less than 50% and
    ...
    def absolute_arrhythmia(self):
        ...
        while self.RRint_dict:
            ...
            for i in xrange(10):
                curDiff = abs(cur_window[i][1] - mean_RR)
                RRwindows.append((curDiff - one_percent,
                                  curDiff + one_percent))
