    ...
    def __ecgpuwave(self, ecg_cur_window):
        with open('filebuf.csv', 'w') as f:
            for i in xrange(len(ecg_cur_window)):
                print(ecg_cur_window[i][1], ecg_cur_window[i][2],
                      sep=',', file=f)
            f.close()

        # wrsamp -F 256 -i filebuf.csv -o filebuf -s ',' 0 1
        subprocess.check_call(["wrsamp", "-F", "256", "-i",
                               "filebuf.csv", "-o", "filebuf",
                               "-s", ",", "0", "1"])
        ...
        with open('filebuf.txt', 'w') as f:
            ...
            subprocess.check_call(["rdann", "-r",
                                   "filebuf", "-a", "ann"], stdout=f)
            f.close()
    ...
    def __ecg_cur_window(self, init_timestamp, ecg_buffer, ecg_cur_window=[]):
        ...
        for i in xrange(__ecgPUrange - lenecg):
            try:
                ...
                ecg_cur_window.append((init_timestamp+i,
                                       ecg_tuple[0], ecg_tuple[1]))
                if i > (__ecgPUrange - lenecg) - __ecgOverlap:
                    ecg_buffer.append((init_timestamp+i,
                                       ecg_tuple[0], ecg_tuple[1]))
            except:
                break
        ...
        with open('filebuf.txt', 'r') as f:
            ...
            for i in xrange(1, len(beatlist)):
                ...
                if cur == 'N':
                    ...
                    if prev_R_peak:
                        self.apcObj.RRint_dict[R_peak_timestamp] =\
                            R_peak_timestamp - prev_R_peak
                        # update prev_R_peak
                        prev_R_peak = R_peak_timestamp

                        # enter QRS interval
                        qrs_interval = next_index - prev_index
                        self.apcObj.QRSwidth_dict[R_peak_timestamp] =\
                            qrs_interval

                        # enter QRS area
                        abssum = 0
                        for j in xrange(prev_index, next_index + 1):
                            abssum += abs(ecg_cur_window[j - 1][1]) +\
                                      abs(ecg_cur_window[j - 1][2])
                        self.apcObj.QRSarea_dict[R_peak_timestamp] = abssum

                        # enter vecg data
                        maxampl, mindex = 0, 0
                        for j in xrange(prev_index, next_index + 1):
                            ampl = sqrt((ecg_cur_window[j - 1][1]**2) +
                                        (ecg_cur_window[j - 1][2]**2))
                            if ampl > maxampl:
                                maxampl, mindex = ampl, j
                        vcg_ang = degrees(atan2(ecg_cur_window[mindex - 1][2],
                                                ecg_cur_window[mindex - 1][1]))
                        if vcg_ang < 0:
                            vcg_ang = 360 + vcg_ang
    ...
    def popluate_aux_structures(self, init_timestamp):
        ...
        try:
            while self.apcObj.ecg_dict:
                ...
                ecg_cur_window, ecg_buffer, init_timestamp =\
                    self.__ecg_cur_window(init_timestamp, ecg_buffer, [])
...
def main():
    ...
    th2 = Thread(target=apcHelperObj.popluate_aux_structures,
                 args=[383021266184])
    th2.start()
    ...
    # plt.plot([key - int(testip[0][0]) for key in
    #           apcHelperObj.apcObj.RRint_dict],
    #          [newarr[key - int(testip[0][0])] for key in
    #           apcHelperObj.apcObj.RRint_dict], 'ro')
    #   plt.show()
