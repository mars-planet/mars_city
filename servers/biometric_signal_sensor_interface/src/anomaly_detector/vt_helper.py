...
# data in different threads through class variables like self.ecg_dict
...
class VTBeatDetector(object):
    ...
    def get_six_second_data(self, start_hexo_time):
        ...
        if start_hexo_time in self.ecg_dict:
            ...
            if (start_hexo_time - (256*6)) in self.ecg_dict:
                ...
                prev_six_second_df = pd.\
                    DataFrame(prev_six_second_dict,
        ...
        six_second_rr_dict = {timestamp: self.
                              ...
                              xrange(__startindex, __endindex + 1)
                              if timestamp in self.rr_dict}
        six_second_rr_dict = sorted(six_second_rr_dict.items())
        six_second_rrint_stat_dict = {timestamp: self.
                                      rr_dict[timestamp][1] for timestamp in
                                      xrange(__startindex, __endindex + 1)
                                      if timestamp in self.rr_dict}
        six_second_rrint_stat_dict = sorted(six_second_rrint_stat_dict.items())

        six_second_rr_df = pd.DataFrame(six_second_rr_dict,
                                        columns=["hexoskin_timestamps",
                                                 "rr_int"])
        six_second_rrint_stat_df = pd.DataFrame(six_second_rrint_stat_dict,
                                                columns=["hexoskin_timestamps",
                                                         "rr_status"])

        return six_second_df, six_second_rr_df,\
            six_second_rrint_stat_df, __ampl
    ...
    def heart_rate_analyzer(self, init_hexo_time):
        ...
        while True:
            try:
                ...
                if not self.hr_low <= self.hr_dict[__timestamp][0]\
                        <= self.hr_high:
                ...
                # delay to be set according to how
                # fast the data is being collected
                # it could be reset dynamically
                # sleep(0.5)
            except:
                return
    ...
    def beat_classf_analyzer(self, init_hexo_time):
        """
        High number of PVC's within a certain time limit can indicate
        a VT onset.
        Many 'unknown' beat types in a short time interval indicate that
        something is wrong with the heart signal because QRS-complexes
        cannot be detected.
        """
        ...
        # print(samplecount)
        # # uncomment to visualize
        # temparr = [((self.ecg_dict[i] * 1.4)/2) for i in
        #            xrange(init_hexo_time, init_hexo_time+(256*25))]
    ...
    def ectopic_analyzer(self, init_hexo_time):
        # look out for a high frequency of ectopic beats
        ...
        try:
            while True:
                ...
                if not ((prev_interval - __thresh_value) <=
                        self.rr_dict[__timestamp][0] <=
                        (prev_interval + __thresh_value)):
                    rrint_q.put(__timestamp)
                    rrintstatus_dict[__timestamp] =\
                        self.rr_dict[__timestamp][1]
                    if rrint_q.full():
                        num_of_zeroone = len([1 for key in rrintstatus_dict if
                                              (rrintstatus_dict[key] == 0 or
                                               rrintstatus_dict[key] == 1)])
                        begin = rrint_q.get()
                        end = __timestamp
                ...
                # delay to be set according to how fast
    ...
    def beat_analyze(self, init_hexo_time):
        # set up threads for heart rate, beat classification and ectopic
        # beats analysis
