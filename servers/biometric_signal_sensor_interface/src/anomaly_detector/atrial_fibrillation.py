...
class AtrialFibrillation(object):
	...
	def __calc_hr_quality(self):
		"""
		returns the percentage of non-zero hr_quality indices in the given range
		"""
		num_of_nonzeros = self.hr_quality_indices['quality_ind'].astype(bool).sum(axis=0)
	...
	def get_anomaly(self):
		...
		end_hexo_timestamp = self.rr_intervals['hexoskin_timestamps'][len(self.rr_intervals)-1]

		self.rr_intervals = self.rr_intervals.reindex(columns=['rr_int', 'hexoskin_timestamps'])
		...
		for i in xrange(1, len(self.rr_intervals)):
			self.rr_intervals['rdr'][i] = self.rr_intervals['rr_int'][i] - self.rr_intervals['rr_int'][i-1]

		__2Dgrid = np.zeros((self.num_of_squares, self.num_of_squares), dtype=np.int8)

		exceptioncnt = 0
		for i in xrange(1, len(self.rr_intervals)):
			# convert seconds to milliseconds and find cell coordinates
			__xindex = int(floor((self.rr_intervals['rr_int'][i] * 1000) / 25)) + self.x_offset
			__yindex = int(floor((self.rr_intervals['rdr'][i] * 1000) / 25)) + self.y_offset
			try:
		...
		if __num_of_nonzero >= self.nec_cutoff_threshold:
			return {'start_hexo_timestamp':start_hexo_timestamp, 'end_hexo_timestamp':end_hexo_timestamp,\
			'num_of_NEC':__num_of_nonzero, 'data_reliability':data_reliability, 'window_size':self.win_size}
		else:
			return None
